...
def scan_wifi_net():
    """Выполняет команду для сканирования Wi-Fi сетей в Windows."""
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            encoding="cp866"  # Кодировка для Windows
        )
...
def percentage_to_dbm(percentage):
    """
    Функция преобразования уровня сигнала из процентов в dBm.
    Диапазон сигналов Wi-Fi примерно от -100 до -30 dBm.
    """
    min_signal = -100  # Минимальный уровень сигнала в dBm
    max_signal = -30   # Максимальный уровень сигнала в dBm
...
def parse_windows_scan_results(lines):
    ...
    for line in lines:
        ...
        # Проверка основных блоков ("SSID X:" и "BSSID Y:")
        ...
        # Обработка уровня сигнала
        if 'Сигнал:' in line:
            signal_match = re.search(r'Сигнал:\s+(\d+)%', line)
            if signal_match:
                signal_percentage = int(signal_match.group(1))
                signal_dbm = percentage_to_dbm(signal_percentage)  # Преобразуем сигнал в dBm
                current_network['signal_strength'] = signal_percentage  # Оставляем сигнал в процентах
                current_network['signal_dbm'] = signal_dbm  # Добавляем сигнал в dBm
                continue

        # Обработка остальных параметров
        additional_params_patterns = {
            'Тип сети': r'Тип сети\s*:\s*(.*)',
            'Проверка подлинности': r'Проверка подлинности\s*:\s*(.*)',
            'Шифрование': r'Шифрование\s*:\s*(.*)',
            'Тип радио': r'Тип радио\s*:\s*(.*)',
            'Канал': r'Канал\s*:\s*(.*)'
        }
    ...
    # Добавляем последнюю сеть, если она осталась
...
def print_results(networks):
    """Печать результатов сканирования Wi-Fi сетей."""
    if not networks:
